...
Companion to **repository root** [`AGENTS.md`](../AGENTS.md). Read **root `AGENTS.md` first**; it explains how **`.planning/`** maps to **`content/docs/<section>/`** (same workflow, different format).
...
| --- | --- | --- |
| **`STATE.xml`** | Current phase, `next-action`, references | Section **`state.mdx`** |
| **`TASK-REGISTRY.xml`** | Tasks, `status`, verification commands | Section **`task-registry.mdx`** |
| **`ROADMAP.xml`** | Phase goals, `status`, `depends` | Phase rows in registries + **[`documentation/roadmap.mdx`](../apps/portfolio/content/docs/documentation/roadmap.mdx)** for human overview |
...
| *What’s next?* | **`STATE.xml`** (`next-action`), **`TASK-REGISTRY.xml`**, relevant **section** `task-registry.mdx` | Pick **one** task; implement; set **`done`**; refresh `next-action` |
...
- **`references`** — root **`AGENTS.md`**, **this file**, **`documentation/requirements.mdx`**, key **`.xml`** paths. No **`IMPLEMENTATION_PLAN.md`**.

## Optional CLI

**`pnpm planning …`** forwards to **`vendor/repo-planner/scripts/loop-cli.mjs`** (see **[integration.mdx](../apps/portfolio/content/docs/repo-planner/integration.mdx)**). Reports default to **`.planning-reports/`**.

## Forbidden (minimal layout)

Do **not** commit **`.planning/REQUIREMENTS.md`**, **`.planning/IMPLEMENTATION_PLAN.md`**, or narrative **`.md`** here except **this** `AGENTS.md`. Upstream templates live under **`vendor/repo-planner/.planning/templates/`** for **full** init only.
