...
- [x] **Planning IA reset (2026-03-30):** section planning docs now live under literal `planning/` folders, one-off section plans belong under `planning/plans/<phase-id>/`, root/operator guidance points at roadmap/state/task-registry/decisions as the living loop, and stale flat planning links were swept across docs, blog, apps, and root planning pointers. Verify: `pnpm run build`, `pnpm run lint`.
...
- [ ] **Reader shell + repub runtime:** [Books task registry](../apps/portfolio/content/docs/books/planning/task-registry.mdx) **`books-reader-03`** -- built-in library shelf in `/apps/reader`, richer manifest metadata for covers/authors/status, Koodo-inspired cover cards, and runtime export from **`@portfolio/repub-builder`**.
...
- [ ] `books-ai-01-10`: live dev `429 insufficient_quota` mismatch (see [books state](../apps/portfolio/content/docs/books/planning/state.mdx)).
