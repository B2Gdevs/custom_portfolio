...
export function NPCNodeV2({ data, selected }) {
    ...
    const contentPreview = node.content.length > 60
        ...
        : node.content;
    return (React.createElement("div", { className: `rounded-lg border-2 transition-all duration-300 ${borderClass} ${isInPath ? 'border-[#e94560]/70' : ''} bg-[#1a1a2e] min-w-[200px] relative`, style: isDimmed ? { opacity: 0.35, filter: 'saturate(0.3)' } : undefined },
        isStartNode && (React.createElement("div", { className: "absolute -top-2 -left-2 bg-green-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shadow-lg z-10" },
            React.createElement(Play, { size: 8, fill: "currentColor" }),
            " START")),
        isEndNode && (React.createElement("div", { className: "absolute -top-2 -right-2 bg-amber-500 text-black text-[8px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shadow-lg z-10" },
            React.createElement(Flag, { size: 8 }),
            " END")),
        React.createElement(Handle, { type: "target", position: targetPosition, className: "!bg-[#2a2a3e] !border-[#4a4a6a] !w-4 !h-4 !rounded-full" }),
        React.createElement("div", { className: `px-3 py-1.5 border-b border-[#2a2a3e] flex items-center gap-2 rounded-t-lg ${headerBgClass}` },
            React.createElement(MessageSquare, { size: 12, className: "text-[#e94560]" }),
            React.createElement("span", { className: "text-[10px] font-mono text-gray-500 truncate flex-1" }, node.id),
            React.createElement("span", { className: "text-[10px] text-gray-600" }, "NPC")),
        React.createElement("div", { className: "px-3 py-2 min-h-[50px]" },
            node.speaker && (React.createElement("div", { className: "text-[10px] text-[#e94560] font-medium mb-1" }, node.speaker)),
            React.createElement("div", { className: "text-xs text-gray-300 line-clamp-2 bg-[#0d0d14] border border-[#2a2a3e] rounded px-2 py-1" },
                "\"",
                contentPreview,
                "\""),
        ...
        React.createElement(Handle, { type: "source", position: sourcePosition, id: "next", className: "!bg-[#2a2a3e] !border-[#4a4a6a] !w-4 !h-4 !rounded-full hover:!border-[#e94560] hover:!bg-[#e94560]/20" })));
}
