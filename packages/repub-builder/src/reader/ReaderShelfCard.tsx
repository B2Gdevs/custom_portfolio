...
export function ReaderShelfCard({
  ...
  const cardClassName = `group relative flex h-full flex-col rounded-xl border border-border bg-card p-5 text-card-foreground shadow-sm transition-[box-shadow,background-color,border-color] hover:border-border hover:bg-muted/15 hover:shadow-md ${
      ...
      {genres.length > 0 ? (
        <div className="flex flex-wrap gap-2" aria-label="Genres">
          {genres.map((g) => (
            <span
              key={g}
              className="rounded-full border border-border bg-muted/60 px-2.5 py-0.5 text-[0.68rem] font-medium text-muted-foreground"
            >
              {g}
            </span>
          ))}
