...
export function parseWorkspaceState(raw: string | null): WorkspaceStateV1 {
  ...
  try {
    ...
    const projects = p.projects.filter(isProject);
    const packs =
      p.packs && typeof p.packs === 'object' && !Array.isArray(p.packs)
        ? (p.packs as Record<string, PlanningPack>)
        : {};
    ...
    return { v: 1, activeProjectId: active, projects, packs };
  } catch {
    return defaultWorkspaceState();
  }
}
