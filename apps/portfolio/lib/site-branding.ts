import { normalizeOptionalTrimmedStringStrict } from '@/lib/coerce-unknown-to-string';
...
export async function getActiveSiteLogoPublicUrl(): Promise<string | null> {
  try {
    ...
    if (!doc) return null;
    const filename = normalizeOptionalTrimmedStringStrict(doc.filename);
    if (filename) return getSiteMediaAssetFileURL(filename);
    return normalizeOptionalTrimmedStringStrict(doc.url);
  } catch {
    return null;
  }
}
