...
import { ArrowUpRight, CalendarDays, Filter, FolderKanban, Search } from 'lucide-react';
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? 'border-accent/70 bg-accent/15 text-primary'
...
export function ContentIndexClient({
  ...
  return (
    <div className="mx-auto flex max-w-7xl gap-8 px-4 py-14 sm:px-6 lg:px-8">
      <aside className="sticky top-24 hidden h-fit w-72 shrink-0 rounded-[2rem] border border-border/70 bg-dark-alt/65 p-5 lg:block">
        <p className="text-xs uppercase tracking-[0.24em] text-text-muted">Explore</p>
        <h2 className="mt-2 font-serif text-2xl text-primary">{kind === 'blog' ? 'Blog Archive' : 'Project Showcase'}</h2>
        <p className="mt-2 text-sm text-text-muted">
          Refine by keyword, tags, and {kind === 'blog' ? 'year' : 'status'} without leaving the archive surface.
        </p>
        <div className="mt-6">
      ...
      <div className="min-w-0 flex-1">
        <div className="rounded-[2rem] border border-border/70 bg-dark-alt/65 p-6 shadow-[0_18px_50px_rgba(0,0,0,0.16)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs uppercase tracking-[0.24em] text-text-muted">
                {kind === 'blog' ? 'Archive' : 'Proof of Work'}
              </p>
            ...
            <div className="flex gap-3 lg:w-[26rem]">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder={kind === 'blog' ? 'Search posts, tags, and sections...' : 'Search projects, proof, and sections...'}
        ...
        <div className="mt-8 space-y-5">
          {results.length === 0 ? (
            <div className="rounded-[2rem] border border-dashed border-border/70 bg-dark-alt/40 p-10 text-center">
              ...
              <p className="mt-2 text-sm text-text-muted">
                Adjust the keyword, clear a filter, or reopen the blog/projects command palette with ⌘K.
              </p>
            </div>
          ) : (
            results.map((item) => {
      ...
      <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
        <SheetContent side="right" className="border-border bg-[#171412] text-primary">
          <SheetHeader>
            ...
            <SheetDescription>
              Refine the {kind === 'blog' ? 'blog archive' : 'projects showcase'} on mobile.
            </SheetDescription>
          </SheetHeader>
