...
const rowHover = 'rounded-md transition hover:bg-muted/35';
...
export function PlanningPackFileTree({
  ...
  async function downloadItemsAsZip(items: PlanningPackItem[], label: string) {
  ...
  return (
    ...
    >
      <div className="border-b border-border/30 px-4 py-3">
        <TabsList className="inline-flex flex-wrap items-center gap-1 rounded-2xl bg-muted/50 p-1 ring-1 ring-inset ring-border/60">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        {loadError ? (
          ...
          <p className="text-sm text-muted-foreground">Loading planning pack exports…</p>
        ) : !activeTab ? (
          ...
          <div className="space-y-3">
            ...
            {grouped.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {activeTab.emptyMessage ?? 'No planning packs available.'}
              </p>
              ...
              grouped.map(([label, group]) => (
                <PackSectionTree
                  ...
                  onDownloadItems={(itemsToDownload, downloadLabel) =>
                    void downloadItemsAsZip(itemsToDownload, downloadLabel)
                  }
      ...
      {contextMenu ? (
        <div
          className="fixed z-[220] min-w-[11rem] max-w-[16rem] overflow-hidden rounded-xl border border-border bg-popover shadow-lg"
          style={{
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <div className="group flex items-center gap-2">
          <CollapsibleTrigger
            type="button"
            className={cn(
              'flex min-w-0 flex-1 items-center gap-2 rounded-lg px-1 py-0.5 text-left transition',
              rowHover,
            )}
            onContextMenu={(event) =>
          ...
          >
            <span
              className={cn(
                ...
                'rounded-md bg-muted/50 text-muted-foreground ring-1 ring-border/40 hover:bg-muted/70 hover:text-foreground',
              )}
            >
              <Download size={13} />
            </span>
...
function TreeDir({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <div className="group">
        <div className="flex items-center gap-2 rounded-md" style={{ paddingLeft: 6 + depth * 14 }}>
          <CollapsibleTrigger
            type="button"
            className={cn(
              'flex min-w-0 flex-1 items-center gap-2 rounded-md px-1.5 py-1 text-left transition',
              rowHover,
            )}
            onContextMenu={(event) =>
          ...
          <HoverDownloadButton label={`Download ${node.name}`} onClick={() => onDownloadItems(node.items, node.name)} />
        </div>
        <CollapsibleContent>
...
function TreeFile({
  ...
  return (
    <div className="group flex items-center gap-2 rounded-md" style={{ paddingLeft: 6 + depth * 14 }}>
      <a
        href={node.item.file}
        download={node.item.filename}
        className={cn(
          'flex min-w-0 flex-1 items-center gap-2 rounded-md px-1.5 py-1 transition',
          rowHover,
        )}
      ...
      {node.item.sizeBytes ? (
        <span className="shrink-0 rounded-full bg-muted/80 px-2 py-0.5 text-[10px] font-semibold text-foreground ring-1 ring-border/50">
...
function HoverDownloadButton({
  ...
  const className = cn(
    buttonVariants({ variant: 'ghost', size: 'icon-xs' }),
    'rounded-md bg-muted/50 text-muted-foreground opacity-0 ring-1 ring-border/40 transition group-hover:opacity-100 focus-visible:opacity-100 hover:bg-muted/70 hover:text-foreground',
  );

  if (href) {
    return (
      <a href={href} download={download} aria-label={label} title={label} className={className}>
        <Download size={13} />
      </a>
    );
  }
