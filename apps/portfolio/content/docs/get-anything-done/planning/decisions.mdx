...
## gad-17: Context window exhaustion: finish, commit, hand off — do not spawn

When a GAD session approaches context limits (signaled by the assistant or by hook warnings), the correct procedure is: (1) finish the current atomic task, (2) update STATE.xml next-action and TASK-REGISTRY.xml task status, (3) run gad sink sync to propagate, (4) commit, (5) start a fresh session. The new session reads gad snapshot or gad context to re-hydrate. This is what the eval framework exists to optimize — the planning docs ARE the handoff.

**Impact:** Agents must not attempt to "push through" context exhaustion by spawning fresh instances, splitting work mid-task, or writing ad-hoc handoff files. The planning loop handles handoff. gad-04 (capture decisions) + gad-17 (handoff via planning docs) are the two halves of the durable-session contract.
