...
└── .planning/
    ...
    ├── *.xml                     # RepoPlanner loop (STATE, TASK-REGISTRY, …) alongside .md
    └── VENDORING.md              # Pointer → docs site vendoring page
```
...
- **IMPLEMENTATION_PLAN.md** — Ralph Wiggum loop state: done, in progress, next. Links to REQUIREMENTS, architecture/design docs on the docs site, vendoring.
- **RepoPlanner XML** — Parallel agent-loop artifacts (`STATE.xml`, `TASK-REGISTRY.xml`, …) via `vendor/repo-planner` and `pnpm planning*`.
- **VENDORING.md** — Short pointer; full submodule docs live under **Documentation → Vendoring** on the site.
