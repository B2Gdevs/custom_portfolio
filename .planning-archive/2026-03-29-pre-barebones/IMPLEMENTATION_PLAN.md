...
| Site AI | CopilotKit + `/api/copilotkit` + layout-gated bubble | AI-v1a-AI-v1d Payload + `sqlite-vec` RAG implementation in progress, then AI-v2 stack evaluation |
...
- [ ] **Books AI implementation:** tracked in [task-registry.mdx](apps/portfolio/content/docs/books/task-registry.mdx) as `books-ai-01-01` through `books-ai-01-08` (Payload collections, sidecar SQLite `sqlite-vec` / FTS search DB, script-first ingest, and AI test coverage are now working; current focus is `books-ai-01-05` citation wiring in the chat UI while the admin trigger stays deferred).
...
- **RAG direction (2026-03-28):** implementation now uses a slim Payload CMS + SQLite backend for canonical RAG records, with `sqlite-vec` and FTS in a sidecar SQLite search DB. Repo content stays canonical; indexed chunks are the retrieval copy. Source scope is docs/projects/blog plus `magicborn` worldbuilding material, ingest is script-first with a deferred admin trigger, admin uses basic auth, embeddings default to `text-embedding-3-small` at `1536` dimensions, retrieval uses an FTS5 BM25 rerank pass, chat reads only from the last completed ingest run, removed sources drop out of the active corpus immediately on commit, and source ids remain stable across reindex runs.
- **RAG verification (2026-03-29):** `pnpm --filter @portfolio/app rag:ingest` now completes successfully against the sidecar RAG DB, indexing `37` sources into `373` chunks. Retrieval smoke tests return cited `magicborn` / docs hits through the read-only search path. Remaining implementation work is chat-side citation rendering and later hardening.
- **AI test coverage (2026-03-29):** app-level `vitest` and `playwright` coverage now exercise the public chat shell launch, Copilot route fallback/delegation, RAG retrieval route validation, source-document corpus rules, and admin basic-auth/deferred-ingest boundary.
